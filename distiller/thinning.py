...
def append_module_directive(thinning_recipe, module_name, key, val):
 ...
 ############################################################################################################
def append_prelu_thinning_directive(thinning_recipe, layers, prelu_name, len_thin_features, thin_features):
    
    prelu_module = layers[prelu_name]
    assert isinstance(prelu_module, torch.nn.modules.activation.PReLU)
    prelu_directive = thinning_recipe.modules.get(prelu_name, {})
    prelu_directive['num_features'] = len_thin_features
    thinning_recipe.modules[prelu_name] = prelu_directive
    
    thinning_recipe.parameters[prelu_name + '.weight'] = [(0, thin_features)]
############################################################################################################
...
def create_thinning_recipe_filters(sgraph, model, zeros_mask_dict):
    ...
    def handle_layer(layer_name, param_name, num_nnz_filters):
        ...
        # Now handle the BatchNormalization layer that follows the convolution
        bn_layers = handle_bn_layers(layers, layer_name, num_nnz_filters, indices)
        ############################################################################################################
        if len(bn_layers) > 0:
             name = bn_layers[0]
             handle_prelu_layers(layers, name, num_nnz_filters, indices)
             handle_bn_layers_bn1(layers, name, num_nnz_filters, indices)
        ############################################################################################################
    ...
    def handle_bn_layers(layers, layer_name, num_nnz_filters, indices):
        bn_layers_back = sgraph.successors_f(layer_name, ['BatchNormalization'])
        if len(bn_layers_back) >= 1:
            # Thinning of the BN layer that follows the convolution
            append_bn_thinning_directive(thinning_recipe, layers, bn_layers_back[0],
                                         len_thin_features=num_nnz_filters, thin_features=indices)
###########################################################################################################
        if layer_name == 'layer4.2.conv2':
            append_bn_thinning_directive(thinning_recipe, layers, 'bn3',
                                         len_thin_features=num_nnz_filters, thin_features=indices)
		
        return bn_layers_back
############################################################################################################
    def handle_bn_layers_bn1(layers, layer_name, num_nnz_filters, indices):
        bn_layers_back_back = sgraph.successors_f(layer_name, ['BatchNormalization'])
        if bn_layers_back_back[0].find('bn1') != -1:
            append_bn_thinning_directive(thinning_recipe, layers, bn_layers_back_back[0],
                                         len_thin_features=num_nnz_filters, thin_features=indices)
 ############################################################################################################
    def handle_prelu_layers(layers, layer_name, num_nnz_filters, indices):
         prelu_layers = sgraph.successors_f(layer_name, ['PRelu'])
  
         if len(prelu_layers) >= 1:
             # Thinning of the PRelu layer that follows the convolution
             append_prelu_thinning_directive(thinning_recipe, layers, prelu_layers[0],
                                         len_thin_features=num_nnz_filters, thin_features=indices)
    ############################################################################################################
