...
class _RankedStructureParameterPruner(_ParameterPruner):
    ...
    def set_param_mask(self, param, param_name, zeros_mask_dict, meta, fpgm=False, HRank=False, conv_index={}):
        ...
        try:
            ...
            model = None
        return self.prune_to_target_sparsity(param, param_name, zeros_mask_dict, fraction_to_prune, model, fpgm, HRank, conv_index)

    def prune_to_target_sparsity(self, param, param_name, zeros_mask_dict, target_sparsity, model, fpgm=False, HRank=False,conv_index={}):
        if not self.is_supported(param_name):
            return
        ...
        if self.group_dependency == "Leader":
            if target_sparsity != self.last_target_sparsity:
                ...
                self.leader_binary_map = self.prune_group(target_sparsity, model.state_dict()[self.leader()],
                                                          self.leader(), zeros_mask_dict=None, fpgm=fpgm, HRank=HRank,conv_index=conv_index)
            assert self.leader_binary_map is not None
            binary_map = self.leader_binary_map
        # Delegate the actual pruning to a sub-class
        self.prune_group(target_sparsity, param, param_name, zeros_mask_dict, model, binary_map, fpgm, HRank=HRank,conv_index=conv_index)

    def prune_group(self, fraction_to_prune, param, param_name, zeros_mask_dict, model=None, binary_map=None, fpgm=False, HRank=False,conv_index={}):
...
class LpRankedStructureParameterPruner(_RankedStructureParameterPruner):
    ...
    def prune_group(self, fraction_to_prune, param, param_name, zeros_mask_dict, model=None, binary_map=None, fpgm=False, HRank=False,conv_index={}):
        ...
        binary_map = group_pruning_fn(fraction_to_prune, param, param_name,
                                      ...
                                      group_size=self.group_size, fpgm=fpgm, HRank=HRank,conv_index=conv_index)
        return binary_map
    ...
    def rank_and_prune_channels(fraction_to_prune, param, param_name=None, zeros_mask_dict=None, 
                                ...
                                noise=0.0, group_size=1, rounding_fn=math.floor, fpgm=False):
    ...
    def rank_and_prune_rows(fraction_to_prune, param, param_name,
                            ...
                            magnitude_fn=l1_magnitude, group_size=1, fpgm=False):
